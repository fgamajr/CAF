...
class AdaptiveScoringPolicy:
    ...
    def _write_feedback_row(self, row: dict[str, object]) -> None:
    ...
    def profile_for(self, query_type: str) -> ScoringProfile:
        ...
        if weights is not None:
            return ScoringProfile(query_type=query_type, weights=weights, source="learned")
        default = DEFAULT_SCORING_PROFILES.get(query_type, DEFAULT_SCORING_PROFILES["factual"])
        return ScoringProfile(query_type=query_type, weights=_normalize_weights(default), source="default")

    def _adjust_weights(self, query_type: str, success: bool) -> dict[str, float]:
    ...
    def register_feedback(
        ...
        *,
        ...
        if query_type not in DEFAULT_SCORING_PROFILES:
            raise ValueError(f"Unsupported query type for scoring feedback: {query_type}")
        updated = self._adjust_weights(query_type, success=success)
    ...
    def feedback_history(self, limit: int = 50) -> list[dict]:
        ...
        rows = [
            json.loads(line)
            for line in settings.scoring_feedback_path.read_text(encoding="utf-8").splitlines()
            if line.strip()
        ]
        return rows[-limit:]
