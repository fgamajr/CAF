...
## 6.1 O dicionário de dados como manual de instruções do CAF
...
50a. A ISO/IEC 11179:2015 estabelece que todo elemento de dados em um registro de metadados deve conter, no mínimo, nome, definição inequívoca, tipo, domínio de valores, unidade de medida (quando aplicável) e fonte autoritativa — o conjunto de atributos necessário para que qualquer usuário autorizado possa interpretar o dado sem conhecimento prévio do sistema. O DAMA-DMBOK v2 complementa esse referencial ao tratar os metadados como pré-requisito para todas as demais funções de governança de dados: sem saber o que cada campo significa, não é possível definir regras de qualidade, implementar validações, planejar integrações nem monitorar a evolução da base (Cap. 12, peça 86, p. 13). A análise que se segue organiza as constatações em dimensões que refletem esses referenciais.

50b. Para manter comparabilidade com os achados anteriores, a equipe adota a mesma escala sintética de maturidade: 0 = ad-hoc; 1 = inicial; 2 = repetível; 3 = definido; 4 = gerenciado; 5 = otimizado. Trata-se de escala analítica adaptada do DAMA-DMBOK e do CMMI-DMM, não de aplicação literal da taxonomia de nenhum desses referenciais.

### Ponto forte: cobertura estrutural

50c. Antes de detalhar as dimensões de fragilidade, é necessário reconhecer o ponto positivo mais relevante: o dicionário de dados do CAF cobre 100% das 95 tabelas efetivamente em uso (peça 134, p. 4). Além disso, 31 campos (5,9%) já apresentam descrições funcionais adequadas (peça 135, p. 4), constituindo benchmark interno que demonstra viabilidade técnica imediata — a equipe do sistema sabe produzir descrições de qualidade quando o faz. O diagnóstico que se segue, portanto, não é sobre ausência de documentação, mas sobre insuficiência qualitativa da documentação existente. Essa distinção é central para calibrar o tom do achado: o CAF dispõe de alicerce concreto para melhoria incremental, não necessita de reconstrução do zero.

### 6.1.1 Diagnóstico por dimensão de qualidade de metadados
...
> Se o dicionário de dados documentasse adequadamente o significado de cada campo, sua unidade de medida e suas relações com outros campos, as validações propostas nos ACH-01, ACH-02 e ACH-03 poderiam ser implementadas como regras de negócio automatizadas, e não como verificações manuais dependentes de quem já conhece o sistema. O ACH-04, portanto, não é apenas mais um achado: é o habilitador transversal cuja resolução potencializa todas as demais melhorias.

## 6.3 Efeitos potenciais
...
## 6.6 Propostas de encaminhamento
